...
### 1. 코랩에서 Ollama 설치 및 실행 순서

코랩 셀(Cell)에 아래 코드를 순서대로 복사해서 실행해 보세요.

**Step 1: Ollama 설치 스크립트 실행**
```bash
!curl -fsSL https://ollama.com/install.sh | sh
```

**Step 2: Ollama 서버 백그라운드 실행**
코랩은 한 셀이 종료될 때까지 다음 셀로 넘어가지 않으므로, 서버를 백그라운드로 띄워야 합니다.
```python
import subprocess
import time
...
**Step 3: 모델 다운로드 및 실행**
이제 원하시는 `llava:7b`나 `gemma` 모델을 내려받아 사용하면 됩니다.
```bash
!ollama pull llava:7b
```
...
* **GPU 가동 필수:** 코랩 상단 메뉴의 **[수정] > [노트북 설정]**에서 하드웨어 가속기를 **T4 GPU**로 설정하세요. CPU로 돌리면 VLM 모델 분석 속도가 매우 느립니다.
* **런타임 연결 끊김:** 코랩은 일정 시간 활동이 없으면 세션이 종료됩니다. 세션이 종료되면 설치했던 Ollama와 모델 파일이 모두 삭제되므로, 다시 접속할 때마다 설치 스크립트를 재실행해야 합니다.
* **라이브러리 활용:** 파이썬에서 더 편하게 제어하려면 `pip install ollama`를 실행한 후 라이브러리를 통해 호출하는 것이 좋습니다.

---

### 3. 코랩 환경에서 메모리 관리 팁
코랩 T4 GPU의 VRAM은 약 **15GB**입니다.
* 7B 모델 한 개를 돌리기엔 충분하지만, 2개를 동시에 올리면 메모리 부족(OOM)이 발생할 수 있습니다.
* 앞서 말씀드린 **`keep_alive=0`** 옵션을 코랩 코드에서도 동일하게 사용하면 메모리 관리에 큰 도움이 됩니다.



현재 로컬 환경에서 **메모리 부족이나 '누텔라 환각' 문제**로 고생 중이시라면, 코랩의 깨끗한 GPU 환경에서 테스트해보시는 것도 좋은 돌파구가 될 것입니다. 코랩용 전체 소스코드가 필요하시면 바로 짜드릴까요?
